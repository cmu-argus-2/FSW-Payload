    ...
    with open(filepath, "rb") as f:
        ...
        while True:
            ...
            if len(payload) < header.payload_size_bytes:
                print(f"[WARNING] Incomplete payload at packet {packet_idx} "
                      f"(expected {header.payload_size_bytes}, got {len(payload)})", file=sys.stderr)
                break
...
def group_fragments_by_tile(packets: List[Packet]) -> Dict[int, List[Tuple[int, bytes]]]:
    ...
    for pkt in packets:
        tiles[pkt.header.tile_idx].append((pkt.header.frag_idx, pkt.payload))
...
def main():
    ...
    if os.path.exists(INPUT_META):
        try:
            ...
            print(f"Loaded metadata: {metadata.tiles_x}x{metadata.tiles_y} tiles, "
    ...
    if metadata is None:
        ...
        print(f"[INFO] Inferred dimensions: {tiles_x}x{tiles_y} tiles, "
