...
public:
    Task(int task_id, std::function<void(Payload*, std::vector<uint8_t>)> func, std::vector<uint8_t> data, Payload* payload, int priority=0);
    // ~Task() = default; // TODO
...
private:
    ...
    std::function<void(Payload*, std::vector<uint8_t>)> func; // task's function to be executed.
